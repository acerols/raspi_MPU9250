...
class SL_MPU9250:
    ...
    def getMag(self):
        ...
        print("row Mag X %d, Y %d" % (rawX, rawY))
        st2     = data[6]
...
if __name__ == "__main__":
    ...
    while True:
        ...
        gyr     = sensor.getGyro()
        mag     = sensor.getMag()
        print("%+8.7f" % acc[0] + " ", end=' ')
        print("%+8.7f" % acc[1] + " ", end=' ')
        print("%+8.7f" % acc[2] + " ", end=' ')
        print(" |   ", end=' ')
        print("%+8.7f" % gyr[0] + " ", end=' ')
        print("%+8.7f" % gyr[1] + " ", end=' ')
        print("%+8.7f" % gyr[2] + " ", end=' ')
        print(" |   ", end=' ')
        print("%+8.7f" % mag[0] + " ", end=' ')
        print("%+8.7f" % mag[1] + " ", end=' ')
        print("%+8.7f" % mag[2])
        ...
        #    MinMagy = mag[1]
#
        #MagOffset[0] = mag[0] - ((MaxMagx + MinMagx) / 2.0)
        #MagOffset[1] = mag[1] - ((MaxMagy + MinMagy) / 2.0)
        #print("Offset Mag x %f, y %f" % ((MaxMagx + MinMagx) / 2.0, (MaxMagy + MinMagy) / 2.0))
        ...
        groll += gpitch * math.sin(gyr[2] * dt * deg2rad)
        gpitch -= groll * math.sin(gyr[2] * dt * deg2rad)
        
        apitch = math.atan2(acc[0], math.sqrt(acc[1] * acc[1] + acc[2] * acc[2])) * rad2deg
        aroll = math.atan2(acc[1], acc[2]) * rad2deg
        
        azimuth = math.atan2(mag[0] + 41.3, mag[1] - 9.5) * rad2deg

        if (yawOffsetFlag == True):
            yawOffset = azimuth

        roll = 0.99 * (roll + gyr[0] * dt) + 0.01 * aroll
        pitch = 0.99 * (pitch + -1.0 * gyr[1] * dt) + 0.01 * apitch
        yaw = 0.99 * (-1.0 * azimuth) + 0.01 * (gyr[2] * dt + yawOffset + yaw) 

        print("groll %f, gpitch %f, gyaw %f" % (groll, gpitch, gyaw))
        print("aroll %f, apitch %f" % (aroll, apitch))
        print("roll %f pitch %f yaw %f" % (roll, pitch, yaw))
        print("azimuth %f" % (-1.0 * azimuth))
